						...
						<div className="fc my-5 w-full items-start gap-3 px-5">
							...
							{movies && (
								<>
									...
									<div className="grid w-full grid-cols-1 xs:grid-cols-2 sm:grid-cols-3 gap-3 md:grid-cols-4 md:gap-5 lg:grid-cols-5 xl:grid-cols-6 place-items-center">
										{movies.length === 0 ? (
											<div className="text-white">No movies added</div>
										) : (
											<>
												{movies.map((movie: MovieDetails) => (
													<div key={movie.id} className="w-full overflow-hidden fc">
														<ContentCard content={movie} dbData={value} />
													</div>
												))}
											</>
										)}
										...
										{tv.length === 0 ? (
											...
											<>
												{tv.map((show: ShowDetails) => (
													<div key={show.id} className="w-full overflow-hidden fc">
														<ContentCard content={show} dbData={value} />
													</div>
												))}
											</>
										)}
